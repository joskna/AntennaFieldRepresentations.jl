...
"""
struct SurfaceCurrentDensity{P <: PropagationType, E <: ElmagType, S <: BEAST.Space{<: Real} , C}  <: AntennaFieldRepresentation{P, C}
   functionspace :: S
...
function Base.similar(s::SurfaceCurrentDensity{P, E, S, C}) where{P, E, S, C}
   return SurfaceCurrentDensity{P, E, S, C}(s.functionspace, similar(s.excitations), s.wavenumber)
end