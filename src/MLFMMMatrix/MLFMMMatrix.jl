...
function MLFMMSource(
    basisfunctions, #Can be ::AbstractSurfaceCurrentDensity, ::AbstractArray{D} where{D<:AbstractDipole}, NamedTuple(:points,:sourcefunctions) 
    wavenumber::R;
    ...
    nodefarfields =
        _allocatenodepattern(FarfieldPattern{Complex{R}}, tree, levelcutoffparameters)
    nodeisfresh = [false for _ = 1:length(MLFMMTrees.tree(tree).nodes)]
    ...
    levelinterpolators = _initializelevelinterpolators(
        R,
        tree,
        θstencilsize,
        ϕstencilsize,
    ...
    phaseshifttoparent =
        _initializephaseshifttoparent(R, tree, levelcutoffparameters, wavenumber)
...
struct MLFMMReceive{
    T<:MLFMMTrees.AbstractMLFMMTree,
    R<:Real,
    A<:AbstractInterpolationStrategy,
}
    tree::T
    # expectedaccuracy::R
    # wavenumber::R
...
function MLFMMReceive(
    testfunctions, #Can be ::AbstractSurfaceCurrentDensity, ::AbstractArray{D} where{D<:AbstractDipole}, NamedTuple(:points,:sourcefunctions)
    ...
    minreceivelevel,
    minsourcetranslationlevel,
    receivetranslationnodes,
    firetranslationnodes =
        _initialize_transferlist(sourcetree, receivetree, num_bufferboxes = num_bufferboxes)
    transferplan = _initializetransferplan(
        transferlist,
        sourcestruct,
        receivetree;
        transfertype = PlannedTransfer{Complex{R}},
    )
    nodeisfresh = [false for _ = 1:length(MLFMMTrees.tree(receivetree).nodes)]
    ...
    levelcutoffparameters =
        _initializelevelcutoffparameters(receivetree, expectedaccuracy, wavenumber)
    ...
    levelcutoffparameters[end-minlevels+1:end] =
        sourcestruct.levelcutoffparameters[end-minlevels+1:end]
    ...
    basisfunctionpatterns = _initializebasisfunctionweightingpatterns(
        receivetree,
        testfunctions,
        levelcutoffparameters,
        wavenumber,
...
function _initialize_tree(
    ...
    return AntennaFieldRepresentations.MLFMMTrees.MLFMMTree(
        root_center,
        points,
        rootsize,
        minhalfsize,
...
"""
function _getsourcefunctions(
    basisfunctions::NamedTuple{(:sourcefunctions, :points),Tuple{V1,V2}},
) where {V1,V2}
...
"""
function _initializelevelcutoffparameters(
    tree::MLFMMTrees.AbstractMLFMMTree,
    expectedaccuracy,
    wavenumber,
)
...
function _allocatenodepattern(
    ...
    transferlist::Vector{Vector{Int}},
) where {R<:Real,S<:Union{PlaneWaveSpectrum{Complex{R}},FarfieldPattern{Complex{R}}}}
    ...
    for level in levels
        ...
        for node in MLFMMTrees.nodesatlevel(tree, level)
            if isempty(transferlist[node])
                nodepattern[node] =
                    P(0, Array{Complex{R}}(undef, 0, 0), Array{Complex{R}}(undef, 0, 0))
            else
                Eθ = Array{Complex{R}}(undef, L + 1, 2L + 2)
                Eϕ = Array{Complex{R}}(undef, L + 1, 2L + 2)
...
"""
function _initializelevelinterpolators(
    R::Type{<:Real},
    tree::MLFMMTrees.AbstractMLFMMTree,
    orderθ::Integer,
    orderϕ::Integer,
    ...
    for level in levels[1:end-1]
        ...
        levelinterpolators[level] =
            interplatortype2(Lold, Lnew, orderθ = orderθ, orderϕ = orderϕ)
...
function _initializebasisfunctionfarfields(
    ...
    k0::R,
    verbose::Bool,
) where {R<:Real,S<:AbstractSurfaceCurrentDensity,I<:Int}
    basisfunctionfarfields =
        Vector{FarfieldPattern{Complex{R}}}(undef, numfunctions(basisfunctions))
    ...
    farfieldmatrices = individualfarfields(
        basisfunctions,
        pts,
        k0,
        quadstrat = BEAST.SingleNumQStrat(1),
        verbose = false,
    )::Matrix{SVector{3,Complex{R}}}
    cost = cos.(θvec)
    sint = sin.(θvec)
    ...
    for functionindex in eachindex(basisfunctionfarfields)
        ...
        basisfunctionfarfields[functionindex] = FarfieldPattern{Complex{R}}(
            L,
            Matrix{Complex{R}}(undef, length(θvec), length(ϕvec)),
            Matrix{Complex{R}}(undef, length(θvec), length(ϕvec)),
        )
...
function _initializebasisfunctionfarfields(
    ...
    k0::R,
    verbose::Bool,
) where {R<:Real,I<:Integer,TT<:MLFMMTrees.AbstractMLFMMTree}

    sourcefunctions = _getsourcefunctions(basisfunctions)
    points = _getpoints(basisfunctions)

    basisfunctionfarfields =
        Vector{FarfieldPattern{Complex{R}}}(undef, length(sourcefunctions))
    L = cutoffparameters[end]
    _, θvec, ϕvec = samplingrule(L)
    nθ = length(θvec)
    nϕ = length(ϕvec)
    ...
    for (f_index, sourcefunction) in enumerate(sourcefunctions)
        basisfunctionfarfields[f_index] = _convertrepresentation_and_resample(
            FarfieldPattern{Complex{R}},
            sourcefunction,
            L,
            k0,
        )
...
function _aggregate_leafnodes!(A::MLFMMSource)
    treetype = MLFMMTrees.PBMLFMMTree{
        AntennaFieldRepresentations.MLFMMTrees.Data{3,Float64},
        3,
        Float64,
    }
...
function _aggregate_children!(A::MLFMMSource, parentnode)
    ...
    for child::Int in ClusterTrees.children(tree, parentnode)
        ...
        _interpolatematrix!(
            ...
            A.nodefarfields[child].Eθ,
            interpolator,
        )
        _muladd!(
            A.nodefarfields[parentnode].Eθ,
            interpolator.finalstorage[Threads.threadid()],
        ...
        _interpolatematrix!(
            interpolator.finalstorage[Threads.threadid()],
            A.nodefarfields[child].Eϕ,
            interpolator,
        )
        _muladd!(
            A.nodefarfields[parentnode].Eϕ,
            interpolator.finalstorage[Threads.threadid()],
            (A.phaseshifttoparent[sector, level+1]),
            reset = reset,
        )
...
function _adjoint_disaggregate_leafnodes!(A::MLFMMReceive)
    ...
    Threads.@threads for leafnode in A.leafnodeindices
        ...
        for functionindex::Int in receivetree(leafnode).data.values::Vector{Int}
            if !reset
                A.nodespectra[leafnode].Eθ .+=
                    A.bvector[functionindex] .*
                    conj.(A.basisfunctionpatterns[functionindex].Eθ)
                A.nodespectra[leafnode].Eϕ .+=
                    A.bvector[functionindex] .*
                    conj.(A.basisfunctionpatterns[functionindex].Eϕ)
            else
                A.nodespectra[leafnode].Eθ .=
                    A.bvector[functionindex] .*
                    conj.(A.basisfunctionpatterns[functionindex].Eθ)
    ...
    Threads.@threads for leafnode in A.leafnodeindices
        ...
        for functionindex::Int in receivetree(leafnode).data.values::Vector{Int}
            _muladd!(
                ...
                A.basisfunctionpatterns[functionindex],
                A.bvector[functionindex],
                reset = reset,
            )
            reset = false
...
function _disaggregate_children!(receivestruct, parentnode)
    ...
    for child::Int in ClusterTrees.children(receivetree, parentnode)
        ...
        _muladd!(
            interpolator.finalstorage[Threads.threadid()],
            receivestruct.nodespectra[parentnode].Eθ,
            receivestruct.phaseshifttoparent[sector, level+1],
            reset = true,
        )
        _adjoint_interpolatematrix!(
            interpolator.finalstorage[Threads.threadid()],
            receivestruct.nodespectra[child].Eθ,
            interpolator,
        ...
        _muladd!(
            interpolator.finalstorage[Threads.threadid()],
            receivestruct.nodespectra[parentnode].Eϕ,
            receivestruct.phaseshifttoparent[sector, level+1],
            reset = true,
        )
        _adjoint_interpolatematrix!(
            interpolator.finalstorage[Threads.threadid()],
            receivestruct.nodespectra[child].Eϕ,
            interpolator,
            reset = !receivestruct.nodeisfresh[child],
        )
...
function _transpose_disaggregate_children!(A, parentnode)
    ...
    for child::Int in ClusterTrees.children(receivetree, parentnode)
        ...
        _interpolatematrix!(
            interpolator.finalstorage[Threads.threadid()],
            A.nodespectra[child].Eθ,
            interpolator,
        )
        _muladd!(
            A.nodespectra[parentnode].Eθ,
            interpolator.finalstorage[Threads.threadid()],
            (A.phaseshifttoparent[sector, level+1]),
            reset = reset,
        )
        ...
        _interpolatematrix!(
            interpolator.finalstorage[Threads.threadid()],
            A.nodespectra[child].Eϕ,
            interpolator,
        )
        _muladd!(
            A.nodespectra[parentnode].Eϕ,
            interpolator.finalstorage[Threads.threadid()],
            (A.phaseshifttoparent[sector, level+1]),
            reset = reset,
        )
...
function _initialize_aggregationlist(TXtree::MLFMMTrees.AbstractMLFMMTree, transferlist)
    ...
    aggregationlist = [Vector{Int}([]) for k = 1:numlevels]
...
"""
function _initialize_transferlist(
    TXtree::MLFMMTrees.AbstractMLFMMTree,
    RXtree::MLFMMTrees.AbstractMLFMMTree;
    num_bufferboxes::Integer = 1,
)
    receivetree = MLFMMTrees.tree(RXtree)
    sourcetree = MLFMMTrees.tree(TXtree)
    ...
    transferlist = [Int[] for _ = 1:MLFMMTrees.numberofnodes(receivetree)]
    adjoint_transferlist = [Int[] for _ = 1:MLFMMTrees.numberofnodes(sourcetree)]
    minreceivelevel::Int = typemax(Int)
    minsourcetranslationlevel::Int = typemax(Int)

    for receivenode in
        MLFMMTrees.DepthFirstIterator(receivetree, MLFMMTrees.root(receivetree))
        ...
        for sourcenode in sourcetree.nodesatlevel[equivalent_sourcelevel]
            ...
            if MLFMMTrees.transferMLFMM(
                ...
                minsourcetranslationlevel = minimum([
                    minsourcetranslationlevel,
                    MLFMMTrees.level(sourcetree, sourcenode) - 1,
                ])
                minreceivelevel = minimum([
                    minreceivelevel,
...
"""
function _initializetransferplan(
    transferlist::Vector{Vector{Int}},
    sourcestruct::MLFMMSource,
    RXtree::MLFMMTrees.AbstractMLFMMTree;
    transfertype::P = PlannedTransfer{Complex{R}},
    ...
    Pℓstorage = [
        Vector{typeof(sourcestruct.wavenumber)}(
            undef,
            sourcestruct.levelcutoffparameters[1] + 1,
    ...
    Threads.@threads for receivenode in eachindex(transferlist)
        ...
        for transfernode in transfers
            Rvec =
                MLFMMTrees.center(receivetree, receivenode) -
                MLFMMTrees.center(sourcetree, transfernode)
            transferplan[receivenode][transfernode] = _initialize_transfer!(
                transfertype,
                Pℓstorage[Threads.threadid()],
                Rvec,
                sourcestruct.wavenumber,
                sourcestruct.levelcutoffparameters[MLFMMTrees.level(
                    sourcetree,
                    transfernode,
                )],
            )
...
function _forward!(sourcestruct::MLFMMSource, receivestruct::MLFMMReceive)
    (sourcestruct.verbose || receivestruct.verbose) &&
