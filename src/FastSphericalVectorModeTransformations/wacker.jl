        ...
        for m in (-ℓ):ℓ
            ...
            for m_ in 0:ℓ
                ...
                for μ in -1:2:1
                    ...
                    u[k, μ_ind] += (1.0im)^(μ - m) * Δℓ[Δm__ind, Δμ_ind] * Δℓ[Δm__ind, Δm_ind] * K[m__ind, m_ind, μ_ind] * (2ℓ + 1) / 2
                end
...
function _receivecoeffs_2by2matrix(u::Array{C,2}, αin::Vector{C}, L::Integer) where {C<:Complex}
...
function _sum_product_αβ(α_inc::Vector{C}, β_aut::Vector{C}, L::Integer; firstorder=true) where {C<:Complex}
...
function _sum_product_αβ_ad(u::Array{C,2}, α_inc::Vector{C}, L::Integer; firstorder=true) where {C<:Complex}
    ## |  u_μℓm = Σ_s α_sℓμ β_sℓm
    ## v 
            ...
            for mθ in (-ℓ):ℓ
                ...
                if firstorder
                    for μ in -1:2:1
                        ...
                        v_[mθind, mind, μind] += Δℓ[Δmθind, Δμind] * Δℓ[Δmθind, Δmind] * u[k, μind] * (1.0im)^(m - μ)
            ...
            for mθ in (-ℓ):ℓ
                ...
                if firstorder
                    for μ in -1:2:1
                        ...
                        u[k, μind] += conj(Δℓ[Δmθind, Δμind] * Δℓ[Δmθind, Δmind] * (1.0im)^(m - μ)) * v_[mθind, mind, μind]
...
function _zeropaddingθ!(v_::Array{C,3}, L::Integer, Jθ::Integer, Lθ::Integer; firstorder=true) where {C<:Complex}
    Nχ = 2
    if !firstorder
        Nχ = 2 * L + 1
    end
...
function _zeropaddingθ_ad(V_::Array{C,3}, L::Integer, Jθ::Integer, Lθ::Integer; firstorder=true) where {C<:Complex}
...
function _zeropaddingϕ!(v::Array{C,3}, L::Integer, Jϕ::Integer, Nθ::Integer; firstorder=true) where {C<:Complex}
    Nχ = 2
    if !firstorder
        Nχ = size(v,3)
    end
...
function _zeropaddingϕ_ad(V::Array{C,3}, L::Integer, Jϕ::Integer, Nθ::Integer; firstorder=true) where {C<:Complex}
    Nχ = 2
    if !firstorder
        Nχ = 2 * L + 1
    end
...
function _zeropaddingχ!(w::Array{C,3}, L::Integer, Jχ::Integer, Jϕ::Integer, Nθ::Integer) where {C<:Complex}
    if 2 * L + 1 > Jχ
        W = zeros(C, Nθ, Jϕ, Jχ)
        if isodd(Jχ)
            Lχ = Int((Jχ - 1) / 2)
...
function _zeropaddingχ_ad(W::Array{C,3}, L::Integer, Jχ::Integer, Jϕ::Integer, Nθ::Integer) where {C<:Complex}
...
function Wackerforward(α_inc::Array{C,1}, β_aut::Array{C,1}, Jθ::Integer, Jϕ::Integer) where {C<:Complex}
    ...
    v_ = _zeropaddingθ!(_expandθmodes(u, L; firstorder=true), L, Jθ, Lθ; firstorder=true)
    v = _zeropaddingϕ!(fft!(v_, 1)[1:Nθ, :, :], L, Jϕ, Nθ; firstorder=true)
    w = fft!(v, 2)
    return _χmodes_to_S12_Firstorder(w)
end

function Wackerforward(α_inc::Array{C,1}, β_aut::Array{C,1}, Jθ::Integer, Jϕ::Integer, Jχ::Integer) where {C<:Complex}
    # Higher-order probe

    L, Nθ, Lθ = _inputdimensions(α_inc, β_aut, Jθ)
    u = _sum_product_αβ(α_inc, β_aut, L; firstorder=false)
    v_ = _zeropaddingθ!(_expandθmodes(u, L; firstorder=false), L, Jθ, Lθ; firstorder=false)
    v = _zeropaddingϕ!(fft!(v_, 1)[1:Nθ, :, :], L, Jϕ, Nθ; firstorder=false)
    w = _zeropaddingχ!(fft!(v, 2), L, Jχ, Jϕ, Nθ)
    return fft(w, 3)
end
function Wackerforward(αin::Array{C,1}, β_aut::Array{C,1}, θvec::Array{F,1}, Jϕ::Integer) where {C<:Complex} where {F<:Number}
...
function Wackerforward(αin::Array{C,1}, β_aut::Array{C,1}, θϕvec::Array{Tuple{N,F},1}) where {C<:Complex,N<:Number,F<:Real}
...
function Wackerforward(αin::Array{C,1}, β_aut::Array{C,1}, θϕvec::Array{Tuple{N,F},1}, k0Δz::Real) where {C<:Complex,N<:Number,F<:Real}
...
function Wackerforward_ad(
    α_inc::Array{C,1}, S12::Array{K,3}, Jθ::Integer, Jϕ::Integer, Jχ::Integer
...
function Wacker(
    S12::Array{C,3}, αin::Array{C,1}, θvec::Array{F,1}, weightvec::Array{K,1}
) where {C<:Complex} where {F<:Real} where {K<:Number}
    ## | Determine size of input data
    ## v   
    a, b, c = size(S12)
    L = a - 1
    b == 2 * L + 2 ? () : throw(DimensionMismatch("Second dimension of S12 must be >=2L+2."))
    c == 2 ? () : throw(DimensionMismatch("Third dimension of S12 must be 2."))
    ## ^
    ## | Determine size of input data
