...
function transfer(pattern::P, tr::OnTheFlyTransfer{T}) where{T,P<:PlaneWaveExpansion{Radiated}}
    # L=transfer.L
...
function transfer(pattern::PlaneWaveExpansion{Radiated}, R::AbstractVector{T}) where{T<:Real}
     tr=OnTheFlyTransfer{pattern.L, T}(SVector{3,T}(R), getwavenumber(pattern))
    return transfer(pattern, tr)
end

function transfer(pattern::PlaneWaveExpansion{Radiated, Y, C}, tr::PlannedTransfer{C}) where{Y, C}
    return PlaneWaveExpansion{Incident, Y, C}( pattern.samplingstrategy, _eθ(pattern).* tr.transfermatrix, _eϕ(pattern) .* tr.transfermatrix , getwavenumber(pattern))
end

function transfer!(incidentfield::P, farfield::F, tr::PlannedTransfer{C}; reset::Bool=true) where{C, F<:PlaneWaveExpansion{Radiated}, P<:PlaneWaveExpansion{Radiated}}
...
# end
function transfer!(incidentfield::PlaneWaveExpansion{Incident}, farfield::PlaneWaveExpansion{Radiated}, R::AbstractVector{T}; reset::Bool=true) where{T<:Real}
...
function _adjoint_translate!(incidentfield::P, farfield::F, tr::PlannedTransfer{C}; reset::Bool=true) where{C, F<:PlaneWaveExpansion{Radiated}, P<:PlaneWaveExpansion{Incident}}
...
function _transpose_translate!(incidentfield::P, farfield::F, tr::PlannedTransfer{C}; reset::Bool=true) where{C, F<:PlaneWaveExpansion{Radiated}, P<:PlaneWaveExpansion{Incident}}
