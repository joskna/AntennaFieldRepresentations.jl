...
struct LocalInterpolation{Lold,Lnew,orderθ,orderϕ,T} <: AbstractInterpolationStrategy where {Lold<:Integer,Lnew<:Integer,orderθ<:Integer,orderϕ<:Integer,T<:Real} end
function Lold(::LocalInterpolation{Lold_,Lnew,orderθ,orderϕ,T}) where {Lold_,Lnew,orderθ,orderϕ,T}
    return Lold_
end
function Lnew(::LocalInterpolation{Lold,Lnew_,orderθ,orderϕ,T}) where {Lold,Lnew_,orderθ,orderϕ,T}
    return Lnew_
end

struct LocalThetaGlobalPhiInterpolation{Lold,Lnew,orderθ,T} <: AbstractInterpolationStrategy where {Lold<:Integer,Lnew<:Integer,orderθ<:Integer,T<:Real} end
function Lold(::LocalThetaGlobalPhiInterpolation{Lold_,Lnew,orderθ,T}) where {Lold_,Lnew,orderθ,T}
    return Lold_
end
function Lnew(::LocalThetaGlobalPhiInterpolation{Lold,Lnew_,orderθ,T}) where {Lold,Lnew_,orderθ,T}
    return Lnew_
end

struct GlobalInterpolation{Lold,Lnew,T} <: AbstractInterpolationStrategy where {Lold<:Integer,Lnew<:Integer,T<:Real} 
...
function GlobalInterpolation{T}(Lold,Lnew) where {T<:Real}
    ...
    sphericalexpansion=RadiatingSphericalExpansion{Complex{T}}(Vector{Complex{T}}(undef, sℓm_to_j(2, Lold, Lold)))
...
function PlannedLocalInterpolation{T}(Lold, Lnew; orderθ=12, orderϕ=12) where {T<:Real}
    ...
    wθ_pos, wθ_neg, θinds_pos, θinds_neg = _planθweightsandindices(θvec_new, θvec_old, orderθ, T)
    ...
    for (k, ϕnew) in enumerate(ϕvec_new)
        ...
        ϕweights[k] = SVector{orderϕ}(T.(lagrange_interpolation_weights_from_barycentric((collect(iϕrange) .- 1) * Δϕ, ϕnew, bϕ)))
        ϕindices[k] = SVector{orderϕ}(map_ϕrange!(iϕrange, length(ϕvec_old)))

    end
    return PlannedLocalInterpolation{orderθ,orderϕ,T}(θweights, θindices, ϕweights, ϕindices, posθranges, negθranges, intermediatestorage, finalstorage, Lold,Lnew)

end
function ParallelPlannedLocalInterpolation{T}(Lold, Lnew; orderθ=12, orderϕ=12) where {T<:Real}
    intermediatestorage = [Array{Complex{T}}(undef, Lnew + 1, 2 * Lold + 2) for k in 1:Threads.nthreads()]
    finalstorage = [Array{Complex{T}}(undef, Lnew + 1, 2 * Lnew + 2) for k in 1:Threads.nthreads()]
    # intermediatestorage=zeros(Complex{T}, Lnew+1, 2*Lold+2)
    θweights = Vector{SVector{orderθ,T}}(undef, Lnew + 1)
    θindices = Vector{SVector{orderθ,Int64}}(undef, Lnew + 1)
    ϕweights = Vector{SVector{orderϕ,T}}(undef, 2 * Lnew + 2)
    ...
    return ParallelPlannedLocalInterpolation{orderθ,orderϕ,T}(θweights, θindices, ϕweights, ϕindices, posθranges, negθranges, intermediatestorage, finalstorage, Lold,Lnew)

end

import Base.show
function Base.show(io::IO, interpolator::PlannedLocalInterpolation{orderθ,orderϕ,T}) where {orderθ,orderϕ,T}
...
function Base.show(io::IO, interpolator::ParallelPlannedLocalInterpolation{orderθ,orderϕ,T}) where {orderθ,orderϕ,T}
...
struct PlannedLocalThetaGlobalPhiInterpolation{Lold,Lnew,orderθ,T} <:
    AbstractInterpolationStrategy where {Lold<:Integer,Lnew<:Integer,orderθ<:Integer,T<:Real}
    θweights::Vector{SVector{orderθ,T}}
...
function PlannedLocalThetaGlobalPhiInterpolation{T}(Lold, Lnew; orderθ=12) where {T<:Real}
    ...
    return PlannedLocalThetaGlobalPhiInterpolation{Lold,Lnew,orderθ,T}(θweights, θindices, fftplan!, ifftplan!, intermediatestorage, finalstorage)
end
function _planθweightsandindices(θvec_new, θvec_old, orderθ, T)
    ...
    for kθ in eachindex(θvec_new)
        wθ_pos_tmp, wθ_neg_tmp, θinds_pos_tmp, θinds_neg_tmp= _split_θ_interpolationparams(θweights[kθ], θindices[kθ], nθ_old)
...
function Lold(::PlannedLocalThetaGlobalPhiInterpolation{Lold_,Lnew,orderθ,T}) where {Lold_,Lnew,orderθ,T}
    return Lold_
end
function Lnew(::PlannedLocalThetaGlobalPhiInterpolation{Lold,Lnew_,orderθ,T}) where {Lold,Lnew_,orderθ,T}
    return Lnew_
end
...
function lagrange_interpolation_weights_from_barycentric(X, t::T, barycentric_weights::Vector{<:Number}) where {T<:Number}
...
function extract_single_planewave(
    Eθmatr::Array{C,2}, Eϕmatr::Array{C,2}, iθrange, wθ::Array{<:Real,1}, iϕrange, wϕ::Array{<:Real,1}
) where {C<:Complex}
...
"""
function _extract_θ_row!(Eθvals, Eϕvals, Eθmatr::Array{C,2}, Eϕmatr::Array{C,2}, iθrange, wθ::AbstractVector) where {C<:Complex}
...
function _extract_single_θ!(storage, Ematr, iθrange, posθrange, negθrange, wθ)# where {C<:Complex}
    ...
    @inbounds mul!(view(storage,:,1:Nϕhalf) , transpose(wθ_neg) , view(Ematr, θinds_neg, Nϕhalf+1:Nϕ),1,1)
    @inbounds mul!(view(storage,:,Nϕhalf+1:Nϕ) , transpose(wθ_neg) , view(Ematr, θinds_neg, 1:Nϕhalf),1,1)
...
function _adjoint_extract_single_θ!(storage, Ematr, iθrange, posθrange, negθrange, wθ)
    ...
    @inbounds for k in eachindex(θinds_pos), kk in 1:Nϕ
        Ematr[θinds_pos[k], kk] +=  wθ_pos[k] * storage[kk]
    end
    @inbounds for k in eachindex(θinds_neg), kk in 1:Nϕhalf
...
function _local_interpolate_ϕ(Eθ_old, Eϕ_old, ϕvec_old::AbstractVector, ϕvec_new::AbstractVector, interpolationorder::Integer)
    ...
    for (k, ϕnew) in enumerate(ϕvec_new)
        ...
        wϕ = lagrange_interpolation_weights_from_barycentric((collect(iϕrange) .- 1) * Δϕ, ϕnew, bϕ)
...
function _planned_upsampleϕ_fft!(Afftnew, Aold, nΦnew, nΦold, fftplan!, ifftplan!)
    ...
    Afftnew[:, (nΦnew - cld(nΦold, 2) + 1):nΦnew] .= view(Afft,:, (nΦold - cld(nΦold, 2) + 1):nΦold)
...
function _planned_downsampleϕ_fft!(Afftnew, Aold, nΦnew, nΦold, fftplan!, ifftplan!)
    ...
    Afftnew[:, (nΦnew - cld(nΦnew, 2) + 1):nΦnew] .= view(Afft,:, (nΦold - cld(nΦnew, 2) + 1):nΦold)
    Afftnew .= ifftplan!*Afftnew
    Afftnew .*= (nΦnew / nΦold)   
    return Afftnew
end
...
function _local_interpolate_θ(Eθ_old, Eϕ_old, θvec_old::AbstractVector, θvec_new::AbstractVector, interpolationorder::Integer)
    nθ_old, nϕ_old = size(Eθ_old)
    ...
    for kθ in 1:maxindex
        ...
        Eθ[kθ, :], Eϕ[kθ, :] = _extract_θ_row!(Eθvals, Eϕvals, Eθ_old, Eϕ_old, (iθrange), wθ)
        Eθ[end - kθ + 1, :], Eϕ[end - kθ + 1, :] = _extract_θ_row!(Eθvals, Eϕvals, Eθ_old, Eϕ_old, iθrange_mirrored, wθ)
    end
    return Eθ, Eϕ
end

function _local_interpolate_θ!(Enew, E_old, Lnew, θindices, posθranges, negθranges, θweights)     
nθ_new = Lnew + 1

    for k in 1:nθ_new
        _extract_single_θ!(transpose(view(Enew, k, :)), E_old, θindices[k], posθranges[k], negθranges[k], θweights[k])
    end
    return Enew
end
...
function _adjoint_local_interpolate_θ!(Enew, E_old, Lnew, θindices, posθranges, negθranges, θweights; reset::Bool=true) 
    nθ_new = Lnew + 1
    if reset 
        E_old .= zero(eltype(E_old))
    end
    for k in 1:nθ_new
        _adjoint_extract_single_θ!(view(Enew, k, :), E_old, θindices[k], posθranges[k], negθranges[k], θweights[k])
    end
    return E_old
end
...
function _interpolatematrix!(storage, oldmatrix, interpolator::PlannedLocalInterpolation{orderθ,orderϕ,T}) where {orderθ,orderϕ,T}
    _local_interpolate_θ!(interpolator.intermediatestorage, oldmatrix, Lnew(interpolator), interpolator.θindices, interpolator.posθranges, interpolator.negθranges, interpolator.θweights)
    _local_interpolate_ϕ!(storage, interpolator.intermediatestorage, Lnew(interpolator), interpolator.ϕindices, interpolator.ϕweights)
    return storage
end
function _interpolatematrix!(oldmatrix, interpolator::PlannedLocalInterpolation{orderθ,orderϕ,T}) where {orderθ,orderϕ,T}
    _interpolatematrix!(interpolator.finalstorage, oldmatrix, interpolator)
    
end
function _interpolatematrix!(storage, oldmatrix, interpolator::ParallelPlannedLocalInterpolation{orderθ,orderϕ,T}) where {orderθ,orderϕ,T}
    _local_interpolate_θ!(interpolator.intermediatestorage[Threads.threadid()], oldmatrix, Lnew(interpolator), interpolator.θindices, interpolator.posθranges, interpolator.negθranges, interpolator.θweights)
    _local_interpolate_ϕ!(storage, interpolator.intermediatestorage[Threads.threadid()], Lnew(interpolator), interpolator.ϕindices, interpolator.ϕweights)
    return storage
end
function _interpolatematrix!(oldmatrix, interpolator::ParallelPlannedLocalInterpolation{orderθ,orderϕ,T}) where {orderθ,orderϕ,T}
    _interpolatematrix!(interpolator.finalstorage[Threads.threadid()], oldmatrix, interpolator)    
end
function _interpolatematrix!(storage, oldmatrix, interpolator::PlannedLocalThetaGlobalPhiInterpolation{orderθ,T}) where {orderθ,T}
        _local_interpolate_θ!(interpolator.intermediatestorage, oldmatrix, Lnew(interpolator), interpolator.θindices, interpolator.posθranges, interpolator.negθranges, interpolator.θweights)
        _planned_resample_fft!(storage, interpolator.intermediatestorage, 2 * Lnew(interpolator) + 2, interpolator.fftplan!, interpolator.ifftplan!)
    return storage
end
function _interpolatematrix!(storage, oldmatrix, ::GlobalInterpolation{Lold,Lnew,T})  where {Lold,Lnew,T}
...
function _adjoint_interpolatematrix!(storage, oldmatrix, interpolator::PlannedLocalInterpolation{orderθ,orderϕ,T}; reset::Bool=true) where {orderθ,orderϕ,T}
    interpolator.intermediatestorage .= _adjoint_local_interpolate_ϕ!(storage, interpolator.intermediatestorage, Lnew(interpolator), interpolator.ϕindices, interpolator.ϕweights)
    oldmatrix .= _adjoint_local_interpolate_θ!(interpolator.intermediatestorage, oldmatrix, Lnew(interpolator), interpolator.θindices, interpolator.posθranges, interpolator.negθranges, interpolator.θweights,  reset=reset)
end
function _adjoint_interpolatematrix!(storage, oldmatrix, interpolator::ParallelPlannedLocalInterpolation{orderθ,orderϕ,T}; reset::Bool=true) where {orderθ,orderϕ,T}
    interpolator.intermediatestorage[Threads.threadid()] .= _adjoint_local_interpolate_ϕ!(storage, interpolator.intermediatestorage[Threads.threadid()], Lnew(interpolator), interpolator.ϕindices, interpolator.ϕweights)
    oldmatrix .= _adjoint_local_interpolate_θ!(interpolator.intermediatestorage[Threads.threadid()], oldmatrix, Lnew(interpolator), interpolator.θindices, interpolator.posθranges, interpolator.negθranges, interpolator.θweights,  reset=reset)
end
...
"""
function resample(
    pattern::FarfieldPattern, interpolator::AbstractInterpolationStrategy)
    # @assert L == Lnew
...
# end
function resample(pattern::FarfieldPattern, ::LocalInterpolation{Lold, Lnew,orderθ,orderϕ, T}) where {Lold, Lnew,orderθ,orderϕ, T}
    @assert pattern.L == Lold
...
function resample(pattern::PlaneWaveSpectrum, ::LocalInterpolation{Lold, Lnew,orderθ,orderϕ, T}) where {Lold, Lnew,orderθ,orderϕ, T}
    @assert pattern.L == Lold
    newpattern = deepcopy(pattern)
    newpattern.L = Lnew
    _, θvec_old, ϕvec_old = samplingrule(Lold)
...
function resample(pattern::PlaneWaveRepresentation, Lnew::Integer)
    return resample(pattern, LocalInterpolation{pattern.L, Lnew, 12,12, real(elementtype(pattern))})
end
function resample(pattern::PlaneWaveRepresentation, ::LocalThetaGlobalPhiInterpolation{Lold, Lnew, orderθ, T}) where {Lold, Lnew, orderθ, T}
    # newpattern = deepcopy(pattern)
    # newpattern.L = L
    _, θvec_old, ϕvec_old = samplingrule(Lold)
    _, θvec_new, ϕvec_new = samplingrule(Lnew)
...
function resample(pattern::FarfieldPattern, interpolator::GlobalInterpolation{Lold,Lnew,T}) where{Lold, Lnew, T}
    @assert pattern.L == Lold
    interpolator.sphericalexpansion .= convertrepresentation(RadiatingSphericalExpansion{Complex{T}}, pattern)
    return _convertrepresentation_and_resample(typeof(pattern), interpolator.sphericalexpansion, Lnew)
end
...
function interpolate_single_planewave(
    θnew::Real, ϕnew::Real, pattern::PlaneWaveRepresentation, ::LocalInterpolation{Lold,Lnew,orderθ,orderϕ,T}
