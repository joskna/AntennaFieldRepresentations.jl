...
In antenna field measurements, the radiated field is usually measured with a so called probe antenna. In general, the measured signal depends on the electromagnetic field incident on the probe antenna (i.e., usually the field of interest that is to be measured) as well as on the receive behaviour of the probe antenna.
In `AntennaFieldRepresentations.jl`, the behaviour of a probe antenna is generally represented by the behaviour of its reciprocal transmit antenna[^2].
...
Any subtype of `FieldSampling` implements the following methods[^3]:
...
[^2]: Most practically relevant antennas are reciprocal, i.e., the reciprocal transmit antenna is identical to the receive antenna. 
[^3]: Note for developers: If you want to implement your own subtype of `FieldSampling`, make sure to support this interface to adhere to the general functionality of this package. 
