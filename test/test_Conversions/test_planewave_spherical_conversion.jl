...
@testset "Conversion PlanewaveSpectrum <-> IncidentSpherical" begin
    ...
    αincref = convertrepresentation(
        IncidentSphericalExpansion{ComplexF64},
        autdipsshifted,
        j_to_sℓm(length(αpwsinc.coefficients))[2],
        k0,
    )

    Jmax = sℓm_to_j(2, 19, 19)
    @test norm(αpwsinc.coefficients[1:Jmax] - αincref.coefficients[1:Jmax]) /
          norm(αincref.coefficients[1:Jmax]) < 1e-5

    @test all(
        isapprox.(
            αpwsinc.coefficients[1:Jmax], αincref.coefficients[1:Jmax]; rtol=1e-5, atol=1e-5
        ),
    )
