...
% ________________________________________________________________________________________________________________
 ...
 \begin{tikzpicture}[very thick] % TikZ picture >>

 \pgfmathsetmacro{\R}{ 3}	
 \draw[ownBlue] (0,0) circle(\R);
 
 \pgfmathsetmacro{\thet}{ 0}
 \foreach \thet in {0,22.5,...,359}
 {
 \pgfmathsetmacro{\x}{ \R * sin(\thet)}
 \pgfmathsetmacro{\y}{ \R * cos(\thet)}
 \filldraw[ownOrange, opacity=0.35] (\x,\y) circle(0.1);
}

 \foreach \thet in {45,67.5,...,157.5}
{
	\pgfmathsetmacro{\x}{ \R * sin(\thet)}
	\pgfmathsetmacro{\y}{ \R * cos(\thet)}
	\filldraw[ownRed] (\x,\y) circle(0.105);
}

\pgfmathsetmacro{\thet}{100}
\pgfmathsetmacro{\x}{ \R * sin(\thet)}
\pgfmathsetmacro{\y}{ \R * cos(\thet)}
\filldraw[ownGreen] (\x,\y) circle(0.1);
